...
flowchart LR
    ...
    WEB -->|"평소: 기사 조회 · 회원/즐겨찾기/검색기록 저장"| DB
    WEB -.->|"예외: 미수집 검색어만, 분당 10회 제한·최대 약 3초·재시도 없음"| NAVER
    USER --> WEB
```

웹 서버(`app.py`)는 **평소에는** 요청을 처리하는 동안 외부 API를 호출하지
않고 DB만 조회·기록합니다. 다만 **한 번도 수집된 적 없는 검색어**를
검색했을 때만 예외적으로, 전체 기준 분당 10회 제한 안에서 네이버 API를
재시도 없이 딱 1회, 최악의 경우에도 약 3초 안에 불러 바로 보여주고
DB에도 저장합니다
(실패하거나 시간 초과면 기다리지 않고 기존의 "아직 수집되지 않은
키워드" 안내로 넘어갑니다). 처음 써보니 "검색했는데 다음 배치까지
최대 1시간을 기다려야 한다"는 게 실사용에서 불편해서 바꿨습니다 —
자세한 배경과 트레이드오프는 트러블슈팅 5.14 참고. 대부분의 기사
수집은 여전히 `collector.py`가 전담하고, GitHub Actions가 주기적으로
(또는 수동으로) 실행시켜 줍니다.

| 파일 | 역할 |
|---|---|
| `app.py` | 웹 서버. 평소엔 DB 조회 전용, 미수집 검색어만 제한된 실시간 API 호출(5.14) |
| `db.py` | Postgres 커넥션 풀 + 스키마 정의 |
| `naver_client.py` | 네이버 API 클라이언트 (재시도/타임아웃 조절 가능, collector.py와 app.py가 함께 사용) |
...
| `gunicorn.conf.py` | gunicorn 워커/스레드 설정(5.14) — 시작 명령을 바꾸지 않아도 자동 적용됨 |
...
- **gunicorn**: Flask 개발 서버 대신 운영용 WSGI 서버. 기본은 sync 워커
  1개라 요청을 하나씩만 처리하는데, 5.14부터 실시간 조회로 한 요청이
  최대 약 3초 묶일 수 있어 `gunicorn.conf.py`로 `gthread` 워커(스레드 4개)
  로 바꿨다. 워커 프로세스를 늘리지 않고 스레드로 늘린 이유는 Render
  512MB 한도 때문(5.14 참고).

## 4. 주요 기능

- 뉴스 검색 / 카테고리별 열람 (평소엔 수집된 기사 조회, 미수집 검색어만 제한된 실시간 조회, 5.14 참고)
...
### 5.14 미수집 검색어를 그 자리에서 실시간 조회하도록 변경

**문제**: 배치/서빙 분리 구조상, 한 번도 검색된 적 없는 키워드는 다음
수집 배치(최대 1시간 뒤, GitHub Actions가 매시 17분 실행)가 돌 때까지
"아직 수집되지 않은 키워드입니다"라는 안내만 보게 된다. 직접 서비스를
써보니 이게 실사용에서 불편했다 — 검색한 사람 입장에서는 "왜 결과가
없지?"에 가깝게 느껴진다.

**해결**: `search_keywords.last_collected_at IS NULL`인 검색어(한 번도
수집된 적 없음)에 한해서만, 결과가 0건일 때 그 자리에서 네이버 API를
호출해 즉시 보여주고 `articles`에도 저장한다(저장 방식은 `collector.py`
의 `collect_one()`과 동일하게 `INSERT ... ON CONFLICT (link) DO
NOTHING`, 성공하면 `last_collected_at`도 갱신). **이미 한 번이라도
수집된 검색어**는 대상이 아니다 — 결과가 없으면 "검색 결과가 없습니다"
로 끝나고, 배치가 계속 최신화한다.

**웹 요청은 배치처럼 느긋하게 기다릴 수 없다는 문제**: 기존
`fetch_naver_news()`는 타임아웃 10초 × 최대 3회 재시도로, 최악의 경우
한 요청이 33초까지 걸릴 수 있었다. 사용자를 그렇게 오래 기다리게 할 수
없어서 `fetch_naver_news()`가 `timeout`/`max_retries`를 인자로 받게
바꾸고, 웹 요청 경로에서는 재시도 0회로 부른다. `collector.py`는 기존
기본값(10초 × 3회)을 그대로 쓴다 — 배치는 재시도할 여유가 있다.

**타임아웃 값을 3초가 아니라 1.5초로 정한 이유**: 처음엔 "3초"로
정했지만, 실제로 응답 없는 주소에 대고 테스트했더니 총 대기 시간이
약 5.9초가 나왔다. `requests`는 숫자 하나짜리 `timeout`을 "연결"과
"응답 대기"에 각각 독립적으로 적용한다(공식 문서에 명시된 동작) —
연결까지는 됐는데 응답이 느리면 연결 대기 + 응답 대기가 더해져
최악의 경우 지정한 값의 약 2배가 걸릴 수 있다. 사용자가 실제로
기다리는 시간을 3초 안팎으로 맞추려고 **1.5초**로 낮췄다(최악의
경우 약 3초).

**실시간 호출이 폭주하지 않도록 전체 기준 분당 10회로 제한한 이유와
방식**: 이 실시간 조회가 몰리면 사실상 API 키 하나로 여러 사용자가
동시에 검색 API를 두드리는 셈이다. gunicorn은 여러 워커 프로세스를
띄울 수 있는데, 각 워커는 메모리를 공유하지 않아서 "프로세스 안 변수에
카운트를 누적"하는 방식으로는 워커 전체를 합산해서 셀 수 없다. 그래서
호출 시각을 `realtime_fetch_log` 테이블에 남기고 "최근 60초 안의 행
수"로 세는 방식을 썼다 — DB는 모든 워커가 공유하므로 프로세스 경계와
무관하게 정확하다. 다만 "개수 확인 후 삽입"을 그냥 하면 두 워커가
동시에 확인해서 둘 다 통과해 버리는 경쟁 상태가 생길 수 있어서,
`pg_advisory_xact_lock`(Postgres의 트랜잭션 범위 락)으로 이 확인+삽입을
전체 워커에서 한 번에 하나씩만 실행되게 직렬화했다. 이 락은 실시간
조회를 시도할 때만(자주 있는 일이 아님) 아주 짧게 걸리므로 성능에
미치는 영향은 무시할 수준이다. 오래된 행은 자리를 예약할 때마다 같이
지우고, `collector.py`도 매시간 한 번씩 별도로 정리한다(아무도 실시간
조회를 시도하지 않는 기간이 길어져도 테이블이 무한정 방치되지 않도록).

**네이버 API를 부르는 동안 DB 커넥션을 붙들지 않도록 분리**: 처음
구현에서는 `with db.connection() as connection:` 블록 안에서 자리
예약과 네이버 호출, 저장까지 전부 처리했다. 커넥션 풀은 최대 10개뿐인데,
네이버 호출은 최악의 경우 몇 초가 걸리는 네트워크 I/O라, 그 몇 초 동안
커넥션 하나가 그냥 대기 상태로 묶여 다른 요청이 커넥션을 못 받을 수
있었다. `fetch_realtime_articles()`(DB 인자를 받지 않음)와
`store_realtime_articles()`(저장 전용)로 나눠서, `home()`이 DB 작업을
전부 끝내고 커넥션을 반납한 뒤에만 네이버를 부르도록 고쳤다. 속도 제한에
쓰는 `pg_advisory_xact_lock`도 자리 예약 함수 안에서 커밋과 함께 바로
풀리므로, 네이버 호출 시점에는 커넥션도 락도 잡고 있지 않다.

**Render의 gunicorn이 기본 sync 워커 1개라 요청이 직렬화되는 문제**:
Render 시작 명령이 `gunicorn app:app`뿐이라 워커/스레드를 따로
지정하지 않으면 sync 워커 1개로 돈다. sync 워커는 요청을 한 번에
하나씩만 처리해서, 실시간 조회로 한 요청이 최대 약 3초 묶이면 그동안
다른 모든 사용자의 요청(로그인, 다른 검색 등)이 전부 대기열에서
기다리게 된다. 저장소 루트에 `gunicorn.conf.py`를 추가해 `worker_class
= "gthread"`, `threads = 4`로 바꿨다. 워커 프로세스를 늘리지 않고
스레드로 늘린 이유는 Render 무료 인스턴스가 512MB로 빠듯해서다(5.6의
재시작 장애도 이 한도 때문이었다) — 워커 프로세스는 늘어난 개수만큼
파이썬 인터프리터와 로드된 라이브러리를 통째로 복제해 메모리를 곱해
쓰지만, 스레드는 같은 프로세스의 힙을 공유하고 스레드 스택만 추가로
든다. 스레드로 바꾸면서 코드 쪽 스레드 안전성도 점검했다: DB 커넥션
풀은 애초에 `ThreadedConnectionPool`이라 문제없고, `get_kiwi()`의
전역 캐시는 이중 검사 잠금을 추가해 경쟁 상태를 막았다(다만 현재
`ENABLE_VISUALIZATIONS=false`라 이 경로 자체가 배포 환경에서는 호출되지
않는다). matplotlib 전역 상태(여러 스레드가 동시에 그리면 꼬일 수 있음)
도 같은 이유로 지금은 해당되지 않지만, 나중에 운영에서 시각화를 켠다면
다시 점검해야 한다.

**한 번 수집된 검색어가 오래 방치되는 문제도 같이 손봤다**:
`search_keywords`에 `last_searched_at`을 추가해 검색할 때마다 갱신하고,
`collector.py`가 "최근 7일 안에 검색됐고 마지막 수집이 6시간 넘은"
검색어를 최대 10개까지 재수집 대상에 포함하도록 했다. 실시간 조회는
"한 번도 수집 안 된" 경우만 처리하므로, 한 번 수집된 뒤 계속 검색되는
키워드가 오래된 기사만 보여주는 상태로 방치되지 않게 하기 위한
보완이다. 이 항목이 늘면서 한 배치가 부를 수 있는 API 호출 수는
최악의 경우(전부 겹치지 않을 때) 고정 카테고리 6 + 인기 검색어 10 +
미수집 검색어 20 + 재수집 대상 10 = **46회**다(`collector.py`의
`MAX_TARGETS_PER_BATCH`).

**트레이드오프**: "웹 서버는 외부 API를 절대 호출하지 않는다"는 이
프로젝트의 원래 원칙을 부분적으로 깬 것이다. `naver_client.py`가 더는
`collector.py` 전용이 아니라 `app.py`도 함께 쓰고, `NAVER_CLIENT_ID`/
`NAVER_CLIENT_SECRET`을 이제 Render 웹 서비스 환경변수에도 설정해야
한다(원래는 GitHub Actions에만 필요했다). 또 미수집 검색어를 검색하는
사용자는 응답이 최대 약 3초 더 걸릴 수 있다. 이 정도 트레이드오프는
"검색했는데 1시간을 기다려야 한다"는 불편함보다 낫다고 판단했다.

**배포 순서 주의**: `record_search()`가 이제 매 검색마다
`search_keywords.last_searched_at`에 값을 쓴다. **운영 DB에
`migrations/2026-09_realtime_search.sql`을 먼저 실행하지 않고 이 코드부터
배포하면, 이 컬럼이 없어서 검색 자체가(신규 키워드뿐 아니라 모든 검색이)
오류로 실패한다.** `init_db()`의 `CREATE TABLE IF NOT EXISTS`는 이미
존재하는 테이블의 컬럼을 추가해 주지 않기 때문이다. 배포 순서는 항상
"① Neon 콘솔에서 마이그레이션 실행 → ② 코드 배포"여야 한다(8번 실행
방법 참고).
...
- 자동 테스트가 없다(이 문서의 테스트는 매번 임시 스크립트로 수행하고 있음).
- 실시간 조회의 분당 10회 제한은 전체 서비스 기준 공동 한도라서, 특정
  시간대에 여러 사용자가 동시에 새 키워드를 검색하면 뒤에 온 사용자는
  실패한 것도 아닌데 대기 없이 바로 "아직 수집되지 않은" 안내를 보게
  된다(다음 배치를 기다려야 함). 사용자별 한도가 아니라 전체 한도라
  생기는 트레이드오프다.
...
- **주요 테이블**: `users`, `articles`, `search_keywords`,
  ...
  `realtime_fetch_log`(5.14, 실시간 조회 속도 제한용)
- **인덱스**:
  ...
  - `realtime_fetch_log(requested_at)` — 최근 60초 안 호출 수 조회용(5.14)
...
| 구성 요소 | 서비스 | 비고 |
|---|---|---|
| 웹 서버 | Render Free Web Service | 15분 무활동 시 슬립, 콜드스타트 있음. Python 3.13(`.python-version`으로 고정). 5.14부터 `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET` 환경변수도 필요(예전엔 GitHub Actions에만 필요했음) |
...
**배포 순서(중요)**: `migrations/`에 스키마 변경 SQL이 쌓여 있는데,
운영 DB에 먼저 적용하지 않고 코드부터 배포하면 깨지는 경우가 있다.
예를 들어 5.14의 `2026-09_realtime_search.sql`을 적용하기 전에 코드를
배포하면, `search_keywords.last_searched_at` 컬럼이 없어서 **신규
검색어뿐 아니라 모든 검색 요청이 오류로 실패한다**(`record_search()`가
매 검색마다 이 컬럼에 쓰기 때문). `init_db()`의
`CREATE TABLE IF NOT EXISTS`는 이미 있는 테이블의 컬럼을 추가해 주지
않으므로, 새 마이그레이션이 생길 때마다 항상 **① Neon 콘솔에서
마이그레이션 SQL 실행 → ② 코드 push/배포** 순서를 지켜야 한다.
